...
export interface Phase2CaseResult {
  ...
  expected_evidence: Array<{ file: string; line: number }>;
...
function metricCounts(expected: Set<string>, actual: Set<string>): DetectionCounts {
...
function mergeCounts(values: DetectionCounts[]): DetectionCounts {
...
function expectedEvidenceMatch(
  scenario: BenchmarkCase,
  result: Awaited<ReturnType<typeof checkRepository>>,
): { file: boolean; line: boolean; actual: SourceEvidence[] } {
  ...
  if (!actual) return { file: false, line: false, actual: [] };
...
function roundedCounts(value: DetectionCounts): DetectionCounts {
  return {
    ...value,
...
export async function evaluatePhase2Benchmark(
  ...
  const groundTruth = JSON.parse(await readFile(absoluteManifest, "utf8")) as BenchmarkGroundTruth;
  ...
  for (const scenario of groundTruth.cases) {
    ...
    try {
      ...
      if (!classificationMatch) issues.push(`${scenario.id}: classification ${actual.classification} != ${scenario.expected.classification}`);
      if (!ruleMatch) issues.push(`${scenario.id}: rule findings [${actualRules.join(", ")}] != [${expectedRules.join(", ")}]`);
      if (!deterministic) issues.push(`${scenario.id}: analyzer output is nondeterministic`);
      if (!evidence.file) issues.push(`${scenario.id}: source evidence file does not match ground truth`);
      if (!evidence.line) issues.push(`${scenario.id}: source evidence line does not match ground truth`);
    } finally {
...
export function formatBenchmarkResult(result: Phase2BenchmarkResult): string {
