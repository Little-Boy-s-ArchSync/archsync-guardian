...
interface HclBlock {
  blockType: "resource" | "module" | "data" | "dynamic";
  labels: string[];
  start: number;
  open: number;
  end: number;
...
interface ParsedLiteral {
  supported: boolean;
  value: InfrastructureAttribute;
}

const resourceKinds = new Map<string, InfrastructureKind>([
...
function scanBlocks(source: string): HclBlock[] {
  ...
  while ((match = pattern.exec(searchable)) !== null) {
    ...
    blocks.push({
      blockType: match[1] as HclBlock["blockType"],
...
function parseLiteral(raw: string): ParsedLiteral {
  ...
  if (/^"(?:[^"\\]|\\.)*"$/.test(value) && !value.includes("${")) {
    return { supported: true, value: JSON.parse(value) as string };
  }
  ...
  if (value.startsWith("[") && value.endsWith("]")) {
    ...
    if (items.every((item) => item.supported && typeof item.value === "string")) {
      return { supported: true, value: items.map((item) => item.value as string) };
    }
  }
  return { supported: false, value };
}

function providerFor(resourceType: string): string {
...
function literalAttributes(
  source: string,
  file: string,
  block: HclBlock,
): { attributes: Record<string, InfrastructureAttribute>; diagnostics: InfrastructureDiagnostic[] } {
  ...
  for (const match of body.matchAll(pattern)) {
    ...
    if (parsed.supported && attribute !== "count" && attribute !== "for_each") {
      ...
      diagnostics.push({
        ...
        severity: "warning",
        message:
          attribute === "count" || attribute === "for_each"
...
function exposureFor(
  attributes: Readonly<Record<string, InfrastructureAttribute>>,
): InfrastructureExposure {
  ...
  if (publicBooleans.some((attribute) => attributes[attribute] === true)) return "public";
  if (attributes.scheme === "internet-facing") return "public";
  const cidrs = attributes.cidr_blocks;
  if (Array.isArray(cidrs) && cidrs.some((cidr) => cidr === "0.0.0.0/0" || cidr === "::/0")) {
    return "public";
  }
  if (publicBooleans.some((attribute) => attributes[attribute] === false)) return "private";
  if (attributes.internal === true || attributes.scheme === "internal") return "internal";
  return "unknown";
}
...
function resourceFromBlock(
  source: string,
  file: string,
  block: HclBlock,
): { resource: ParsedInfrastructureResource; diagnostics: InfrastructureDiagnostic[] } {
  ...
  const blockEvidence = evidenceAt(
    source,
    file,
    block.start,
    block.end + 1,
  ...
  return {
    resource: {
      ...
      attributes: Object.fromEntries(
        Object.entries(parsed.attributes).sort(([left], [right]) => left.localeCompare(right)),
      ),
      evidence: [blockEvidence],
    },
...
export function parseTerraform(source: string, file = "main.tf"): InfrastructureParseResult {
