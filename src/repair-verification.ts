...
export type RepairRecheck = (
  workspace: string,
  stage: "BASELINE" | "CANDIDATE",
) => Promise<RepairConformanceSnapshot>;
...
function failure(code: RepairSafetyCode, message: string): PatchValidationFailure {
...
function normalizedRepairPath(value: string): string | PatchValidationFailure {
  if (
    value.length === 0 ||
    value.length > 512 ||
    value.includes("\0") ||
...
export function resolveSandboxPath(root: string, relativePath: string): string {
  const normalized = normalizedRepairPath(relativePath);
  if (typeof normalized !== "string") throw new Error(`${normalized.code}: ${normalized.message}`);
  return resolve(root, ...normalized.split("/"));
}

export function shouldCopySandboxEntry(relativePath: string): boolean {
...
export async function createRepairSandbox(
  sourceRoot: string,
  options: {
    temp_parent?: string;
  ...
  if (!(await stat(source)).isDirectory()) throw new Error("Repair source root must be a directory");
...
function parsePatchHeaderPath(value: string, prefix: "a/" | "b/"): string | undefined {
  if (value === "/dev/null") return value;
  if (value.includes("\t") || value.startsWith('"') || !value.startsWith(prefix)) return undefined;
  return value.slice(2);
}

function patchSections(unifiedDiff: string): string[][] | undefined {
  const lines = unifiedDiff.replace(/\r\n/gu, "\n").split("\n");
  const starts = lines.flatMap((line, index) => line.startsWith("diff --git ") ? [index] : []);
  if (starts.length === 0 || lines.slice(0, starts[0]).some((line) => line !== "")) return undefined;
  return starts.map((start, index) => lines.slice(start, starts[index + 1] ?? lines.length));
}

function validatePatchSection(section: string[]): string | PatchValidationFailure {
...
export function validateRepairCandidate(candidate: RepairCandidate): PatchValidationResult {
  ...
  const expectedPaths = new Map<string, RepairFileExpectation>();
  ...
  if (!sections) return failure("UNSUPPORTED_PATCH", "A repair must be a git-style unified diff");
  const paths: string[] = [];
  for (const section of sections) {
    const path = validatePatchSection(section);
    if (typeof path !== "string") return path;
    if (paths.includes(path)) return failure("UNSUPPORTED_PATCH", `Path '${path}' appears more than once`);
    paths.push(path);
  }
...
async function assertNoSymlinkSegments(root: string, path: string): Promise<void> {
  ...
  for (let index = 0; index < segments.length; index += 1) {
    ...
    try {
      ...
      if (metadata.isSymbolicLink()) throw new Error(`SYMLINK_PATH: '${path}' crosses a symbolic link`);
...
async function fileSha256(root: string, path: string): Promise<string | null> {
  await assertNoSymlinkSegments(root, path);
  const absolute = resolveSandboxPath(root, path);
  try {
    const metadata = await lstat(absolute);
    if (!metadata.isFile()) throw new Error(`DIRTY_WORKSPACE: '${path}' is not a regular file`);
...
export async function executeProcess(invocation: ProcessInvocation): Promise<ProcessResult> {
  try {
...
export function networkSandboxInvocation(
...
export function createPlatformNoNetworkExecutor(
  platform: NodeJS.Platform = process.platform,
  runner: ProcessRunner = executeProcess,
): NoNetworkCommandExecutor | undefined {
  if (!networkSandboxInvocation(platform, "probe", [])) return undefined;
  return {
    network_isolation: "ENFORCED",
    execute: async (invocation) => {
      ...
      if (
        (platform === "linux" && /^unshare: .*Operation not permitted/imu.test(result.stderr)) ||
...
export function sanitizeVerificationLog(
  value: string,
  sandboxRoot: string,
...
function commandIsAllowed(command: string, allowlist: readonly string[]): boolean {
...
export async function runSandboxCommand(
  ...
  const output = {
    stdout: sanitizeVerificationLog(
      result.stdout,
      sandbox.root,
      options.sensitive_values,
    ),
    stderr: sanitizeVerificationLog(
      result.stderr,
      sandbox.root,
      options.sensitive_values,
    ),
...
export async function detectProjectTestCommand(workspace: string): Promise<SandboxCommand | undefined> {
  const packagePath = join(workspace, "package.json");
  if (!(await exists(packagePath))) return undefined;
  let manifest: { scripts?: Record<string, string> };
  try {
    manifest = JSON.parse(await readFile(packagePath, "utf8")) as { scripts?: Record<string, string> };
  } catch {
    return undefined;
  }
  if (!manifest.scripts?.test) return undefined;
  if (await exists(join(workspace, "pnpm-lock.yaml"))) return { command: "pnpm", args: ["test"] };
  if (await exists(join(workspace, "yarn.lock"))) return { command: "yarn", args: ["test"] };
...
export function gitEnvironment(platform: NodeJS.Platform = process.platform): NodeJS.ProcessEnv {
  return {
...
export async function applyRepairCandidate(
  sandbox: RepairSandbox,
  candidate: RepairCandidate,
  runner: ProcessRunner = executeProcess,
  ...
  try {
    const common: Omit<ProcessInvocation, "args"> = {
      command: "git",
      cwd: sandbox.workspace,
      env: gitEnvironment(),
      timeout_ms: 30_000,
    ...
    for (const path of validation.paths) after[path] = await fileSha256(sandbox.workspace, path);
...
export function guardianFindingFingerprint(finding: GuardianFinding): string {
...
export function guardianResultToRepairSnapshot(result: GuardianResult): RepairConformanceComplete {
...
export function normalizeRepairConformanceSnapshot(
  snapshot: RepairConformanceSnapshot,
): RepairConformanceSnapshot {
...
async function safeRecheck(
  ...
  try {
    ...
    return {
      status: "ERROR",
      message: sanitizeVerificationLog((error as Error).message, workspace, sensitiveValues),
    };
  }
}

export function compareRepairConformance(
  targets: string[],
  baseline: RepairConformanceComplete,
  candidate: RepairConformanceComplete,
): RepairConformanceComparison {
...
export function decideRepairVerification(input: RepairDecisionInput): RepairDecision {
...
function emptyPatchFailure(
  candidate: RepairCandidate,
  validation: PatchValidationFailure,
...
export async function verifyRepairCandidate(
  ...
  if (!validation.ok) return emptyPatchFailure(options.candidate, validation);
  let sandbox: RepairSandbox;
  try {
  ...
  let result: RepairVerificationResult;
  try {
    const baseline = await safeRecheck(
      options.recheck,
      sandbox.workspace,
      "BASELINE",
      options.sensitive_values ?? [],
    ...
    if (baseline.status === "ERROR") {
      ...
      if (missingTargets.length > 0) {
        const comparison = compareRepairConformance(
          options.candidate.target_block_finding_fingerprints,
          baseline,
          baseline,
        );
        result = {
        ...
        const patch = await applyRepairCandidate(
          sandbox,
          options.candidate,
          options.process_runner ?? executeProcess,
        );
        if (patch.status !== "APPLIED") {
          ...
          const candidate = await safeRecheck(
            options.recheck,
            sandbox.workspace,
            "CANDIDATE",
            options.sensitive_values ?? [],
    ...
    result = {
      ...
      candidate_id: options.candidate.candidate_id,
      decision: "INCONCLUSIVE",
      reason: sanitizeVerificationLog(
  ...
  try {
    ...
    return {
      ...result,
      decision: "INCONCLUSIVE",
      reason: `Sandbox cleanup failed: ${(error as Error).message}`,
