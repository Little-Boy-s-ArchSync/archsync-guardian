import {
  ...
  infrastructureGraphVersion,
  type ClassifiedEvidenceClaim,
  type ConflictClassification,
  type CrossSourceEvidenceClaim,
  type CrossSourceIdentityMap,
  type EvidenceClaimProperty,
  type EvidenceClaimValue,
  type EvidenceSourceClass,
  type IdentityAliasRule,
  type IdentityResolution,
  type InfrastructureDiagnostic,
  type InfrastructureEvidence,
  type InfrastructureExposure,
  type InfrastructureKind,
  type InfrastructureObservation,
  type InfrastructureParseResult,
  type NormalizedInfrastructureEdge,
  type NormalizedInfrastructureGraph,
  type NormalizedInfrastructureNode,
  type ParsedInfrastructureReference,
} from "./iac-contracts.js";

const sourceOrder: Record<EvidenceSourceClass, number> = { spec: 0, code: 1, iac: 2 };
...
function observationKey(observation: InfrastructureObservation): string {
  return `${observation.source}:${observation.native_id}`;
}

function stableEvidence(evidence: readonly InfrastructureEvidence[]): InfrastructureEvidence[] {
  const unique = new Map<string, InfrastructureEvidence>();
...
function canonicalUnknown(observation: InfrastructureObservation, prefix = "unknown"): string {
...
function aliasMatches(
  ...
  rule: IdentityAliasRule,
): boolean {
  const namespace = normalizedToken(observation.namespace);
  const ruleNamespace = normalizedToken(rule.namespace);
  if (ruleNamespace !== "any" && namespace !== ruleNamespace) return false;
  const candidates = new Set(
    [observation.name, observation.native_id, ...observation.aliases].map(normalizedToken),
  );
  return rule.aliases.some((alias) => candidates.has(normalizedToken(alias)));
}

export function observationsFromParseResult(
  result: InfrastructureParseResult,
...
export function mapCrossSourceIdentities(
  observations: readonly InfrastructureObservation[],
  aliasRules: readonly IdentityAliasRule[] = [],
): CrossSourceIdentityMap {
  const ordered = [...observations].sort((left, right) =>
    observationKey(left).localeCompare(observationKey(right)),
  );
  const byNamespaceName = new Map<string, InfrastructureObservation[]>();
  const byName = new Map<string, InfrastructureObservation[]>();
  ...
  for (const observation of ordered) {
    ...
    const hasUnscopedName = sameName.some(
      (candidate) =>
        normalizedToken(candidate.namespace) === "" || candidate.namespace === "unknown",
    );
    if (
      sameName.length > 1 &&
  ...
  return {
    ...
    resolutions,
    unknowns: resolutions.filter(
      ({ method }) => method === "unknown" || method === "ambiguous",
    ),
  };
}
...
function preferredKind(observations: readonly InfrastructureObservation[]): InfrastructureKind {
...
function preferredBoundary(observations: readonly InfrastructureObservation[]): string {
...
function normalizedNode(
  ...
  for (const observation of ordered) {
    ...
    for (const alias of observation.aliases) aliases.add(alias);
  }
  return {
    id,
    ...
    sources: [...new Set(ordered.map(({ source_class }) => source_class))].sort(
...
function mappedEndpoint(
  nativeId: string,
  resolutionByObservation: ReadonlyMap<string, IdentityResolution>,
): string {
...
function normalizedEdges(
  references: readonly ParsedInfrastructureReference[],
  resolutions: readonly IdentityResolution[],
): NormalizedInfrastructureEdge[] {
  const resolutionMap = new Map(resolutions.map((resolution) => [resolution.observation_key, resolution]));
  const accumulators = new Map<string, NormalizedInfrastructureEdge>();
...
export function buildNormalizedInfrastructureGraph(
  observations: readonly InfrastructureObservation[],
  references: readonly ParsedInfrastructureReference[],
  diagnostics: readonly InfrastructureDiagnostic[] = [],
  aliasRules: readonly IdentityAliasRule[] = [],
  ...
  const resolutionByKey = new Map(
    identityMap.resolutions.map((resolution) => [resolution.observation_key, resolution]),
  );
  const grouped = new Map<string, InfrastructureObservation[]>();
...
function propertyValue(
  ...
  if (property === "identity") return canonicalId;
  if (property === "kind") return observation.kind;
  if (property === "exposure") return observation.exposure;
  return observation.trust_boundary;
}

export function buildCrossSourceEvidenceClaims(
  graph: NormalizedInfrastructureGraph,
): CrossSourceEvidenceClaim[] {
  const properties: EvidenceClaimProperty[] = ["identity", "kind", "exposure", "trust-boundary"];
  const required: EvidenceSourceClass[] = ["spec", "code", "iac"];
  const claims: CrossSourceEvidenceClaim[] = [];
  for (const node of graph.nodes) {
    for (const property of properties) {
      const grouped = new Map<string, EvidenceClaimValue>();
...
function classificationFor(claim: CrossSourceEvidenceClaim): {
...
export function classifyEvidenceClaim(
  claim: CrossSourceEvidenceClaim,
): ClassifiedEvidenceClaim {
  return { ...claim, ...classificationFor(claim) };
}

export function classifyEvidenceClaims(
  claims: readonly CrossSourceEvidenceClaim[],
): ClassifiedEvidenceClaim[] {
  return [...claims]
    .sort((left, right) => left.id.localeCompare(right.id))
