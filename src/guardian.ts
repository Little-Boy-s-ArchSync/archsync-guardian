...
function sourceEvidenceKey(value: SourceEvidence): string {
...
function uniqueEvidence(values: SourceEvidence[]): SourceEvidence[] {
  return [...new Map(values.map((value) => [sourceEvidenceKey(value), value])).values()].sort((a, b) =>
    sourceEvidenceKey(a).localeCompare(sourceEvidenceKey(b)),
  );
}
...
function preferredComponentEvidence(
  ...
  return [...(anchors.length > 0 ? anchors : candidates)].sort((a, b) =>
    evidenceRank(a) - evidenceRank(b) || sourceEvidenceKey(a).localeCompare(sourceEvidenceKey(b)),
  ).slice(0, 1);
}

function findingSourceEvidence(
...
export function evaluateObservedArchitecture(
...
export async function checkRepository(
  ...
  return evaluateObservedArchitecture(
    expected,
    await analyzeTypeScriptRepository(repositoryPath, expected),
  );
}

export function formatGuardianResult(result: GuardianResult): string {
  const lines = [
  ...
  for (const finding of result.findings) {
    ...
    lines.push(
      `- [${finding.id}] ${finding.severity.toUpperCase()} ${finding.kind} at ${locations}: ${finding.message}`,
    );
  }
  ...
  lines.push(
    `OBSERVED ${Object.keys(result.observed.components).length} components, ${result.observed.relationships.length} relationships, ${result.observed.metadata.scanned_files} TypeScript files`,
  );
  return lines.join("\n");
}
