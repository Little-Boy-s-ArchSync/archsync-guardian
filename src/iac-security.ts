...
function stableEvidence(evidence: readonly InfrastructureEvidence[]): InfrastructureEvidence[] {
  const values = new Map<string, InfrastructureEvidence>();
...
function findingId(rule: string, subject: string): string {
  return `${rule}:${subject}`.toLowerCase().replaceAll(/[^a-z0-9:|./_-]+/g, "-");
}

function nodesById(graph: NormalizedInfrastructureGraph): Map<string, NormalizedInfrastructureNode> {
  return new Map(graph.nodes.map((node) => [node.id, node]));
}

export function evaluateInfrastructureSecurity(
  ...
  for (const node of graph.nodes) {
    ...
    const isDatabase =
      node.kind === "database" || node.observations.some(({ kind }) => kind === "database");
    if (
      isDatabase &&
    ...
    const hasPublicIngress = node.observations.some(
      ({ kind, exposure }) => kind === "ingress" && exposure === "public",
    );
    if (
      (node.kind === "ingress" && node.exposure === "public" || hasPublicIngress) &&
      !node.approved
    ) {
      findings.push({
  ...
  for (const edge of graph.edges) {
    if (edge.type !== "routes-to" && edge.type !== "connects-to") continue;
    if (!edge.resolved) continue;
    const from = index.get(edge.from);
    const to = index.get(edge.to);
    if (!from || !to) continue;
    if (
      from.trust_boundary === "unknown" ||
