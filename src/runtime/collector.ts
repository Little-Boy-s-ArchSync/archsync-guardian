...
import {
  runtimeCollectorVersion,
  runtimeEvidenceContractVersion,
  type NormalizedRuntimeSpan,
  type OtlpAttribute,
  type OtlpExport,
  type OtlpMetricPoint,
  type OtlpScalar,
  type RuntimeCollectionOptions,
  type RuntimeEvidenceSnapshot,
  type RuntimeGoalMeasurement,
...
function array(value: unknown, path: string): unknown[] {
  if (!Array.isArray(value)) throw new RuntimeEvidenceError(path, "expected an array");
  return value;
}
...
function finiteNumber(value: unknown, path: string): number {
  ...
  if (!Number.isFinite(parsed)) throw new RuntimeEvidenceError(path, "expected a finite number");
  return parsed;
}

function scalar(value: unknown, path: string): OtlpScalar {
  ...
  if (key === "stringValue") return text(wrapped[key], `${path}/${key}`);
  if (key === "boolValue") {
...
function attributeMap(value: unknown, path: string): Map<string, OtlpScalar> {
  const output = new Map<string, OtlpScalar>();
  array(value, path).forEach((raw, index) => {
    ...
    if (output.has(key)) throw new RuntimeEvidenceError(`${path}/${index}/key`, "duplicate attribute key");
    output.set(key, scalar(item.value, `${path}/${index}/value`));
  });
  return output;
}

function requiredAttribute(attributes: Map<string, OtlpScalar>, key: string, path: string): string {
  return text(attributes.get(key), `${path}/${key}`);
}

function nano(value: unknown, path: string): string {
  const parsed = text(value, path);
  if (!decimalNano.test(parsed)) throw new RuntimeEvidenceError(path, "expected unsigned unix nanoseconds");
  return parsed;
}

function withinWindow(timestamp: string, options: RuntimeCollectionOptions, path: string): void {
...
function validateOptions(options: RuntimeCollectionOptions): void {
...
function resourceIdentity(value: unknown, path: string, options: RuntimeCollectionOptions): string {
  ...
  const environment = requiredAttribute(
    attributes,
    "deployment.environment.name",
    `${path}/attributes`,
  );
...
function direction(kind: number): NormalizedRuntimeSpan["direction"] {
  if (kind === 3 || kind === 4) return "outbound";
  if (kind === 2 || kind === 5) return "inbound";
  return "internal";
}

function relationship(attributes: Map<string, OtlpScalar>): RuntimeRelationshipType {
  if (attributes.has("db.system")) return "data";
  if (attributes.has("messaging.system")) return "async";
  if (attributes.has("http.request.method") || attributes.has("rpc.system")) return "http";
  return "dependency";
}

function peer(attributes: Map<string, OtlpScalar>): string | undefined {
...
function normalizeSpan(
  ...
  path: string,
  service: string,
  options: RuntimeCollectionOptions,
...
function parseExport(value: unknown, options: RuntimeCollectionOptions): {
  spans: NormalizedRuntimeSpan[];
  measurements: RuntimeGoalMeasurement[];
} {
  const root = record(value, "/");
  const spans: NormalizedRuntimeSpan[] = [];
  const measurements: RuntimeGoalMeasurement[] = [];
      ...
      array(scope.metrics, `${scopePath}/metrics`).forEach((rawMetric, metricIndex) => {
        ...
        array(gauge.dataPoints, `${metricPath}/gauge/dataPoints`).forEach((point, pointIndex) => {
          measurements.push(normalizeMeasurement(
            point,
            `${metricPath}/gauge/dataPoints/${pointIndex}`,
            service,
            name,
            unit,
            options,
...
export function serializeRuntimeEvidence(snapshot: RuntimeEvidenceSnapshot): string {
