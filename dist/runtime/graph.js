...
function validateMappingConfig(config) {
    ...
    for (const [service, ids] of Object.entries(config.services)) {
        if (service.length === 0)
            throw new RuntimeEvidenceError("/mapping/services", "service name must not be empty");
...
function mappingFor(service, config) {
    const ids = [...(config.services[service] ?? [])].sort();
    if (ids.length === 1)
        return { ids, status: "mapped", confidence: 1 };
    if (ids.length > 1)
        return { ids, status: "ambiguous", confidence: 0 };
    return { ids, status: "unmapped", confidence: 0 };
}
function evidenceForService(service, snapshot) {
    return [
        ...snapshot.spans
...
function timestampBounds(service, snapshot) {
    const timestamps = [
...
function edgeEndpoints(span) {
    if (span.peer_service === undefined || span.direction === "internal")
        return undefined;
...
function combinedStatus(from, to) {
    if (from.status === "unmapped" || to.status === "unmapped")
        return "unmapped";
    if (from.status === "ambiguous" || to.status === "ambiguous")
        return "ambiguous";
    return "mapped";
}
export function buildObservedRuntimeGraph(snapshot, config) {
    ...
    const nodes = snapshot.services.map((service) => {
        ...
        return {
            ...
            span_count: snapshot.spans.filter((span) => span.service === service || span.peer_service === service).length,
            metric_count: snapshot.measurements.filter((measurement) => measurement.service === service).length,
    ...
    const modelComponents = [...config.expected_components].sort().map((componentId) => {
        ...
        return {
            ...
            observation: runtimeServices.length === 0 ? "no-evidence" : "observed",
