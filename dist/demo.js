...
import { checkRepositoryDiff, formatPhase3Result } from "./phase3.js";
...
async function resolveBenchmark(input) {
    if (input)
        return resolve(input);
    const cwd = process.cwd();
    if (await exists(join(cwd, "ground-truth.json")))
        return cwd;
    if (await exists(join(cwd, "order-platform", "ground-truth.json"))) {
        return join(cwd, "order-platform");
    }
    throw new Error("Benchmark not found. Run this command from archsync-benchmark or pass --benchmark <order-platform-directory>.");
}
async function chooseScenario(manifest) {
...
`);
        ...
        return { "1": "pass", "2": "block", "3": "review", "4": "all" }[answer]
...
function scenarioForAlias(manifest, alias) {
    const aliases = {
        pass: "no-impact",
        block: "violation",
        review: "evolution",
    ...
    if (alias === "all") {
        return ["no-impact", "violation", "evolution"].map((category) => {
            ...
            if (!scenario)
                throw new Error(`Benchmark has no ${category} demo case`);
            return scenario;
        });
    }
...
async function runScenario(benchmark, manifest, scenario) {
...
export async function runDemo(options = {}) {
    ...
    const manifest = JSON.parse(await readFile(join(benchmark, "ground-truth.json"), "utf8"));
    const selected = options.scenario ?? (options.interactive && process.stdin.isTTY ? await chooseScenario(manifest) : "all");
...
export function formatDemoResult(result, verbose = false) {
...
export function formatDemoMarkdown(result) {
    return [
        ...
        "| --- | --- | --- | --- | --- | --- |",
        ...result.cases.map((demoCase) => `| ${demoCase.case_id} | ${demoCase.expected_decision} | ${demoCase.actual_decision} | ${demoCase.cache.cold} -> ${demoCase.cache.warm} | ${firstEvidence(demoCase.result)} | ${demoCase.match ? "YES" : "NO"} |`),
    ].join("\n");
}
