...
async function writeText(output, value) {
    const outputPath = resolve(output);
    await mkdir(dirname(outputPath), { recursive: true });
...
async function main() {
    ...
    if (modelAliases.has(command) || command === "validate-benchmark") {
        const exitCode = await runModelCommand(command === "validate-benchmark" ? "benchmark" : command, commandArgs);
        process.exitCode = exitCode ?? usage();
        return;
    }
    ...
    if (command === "check" || command === "check-json") {
        ...
        if (args.includes("--diff")) {
            ...
            if (report)
                await writeText(report, formatPhase3Markdown(result));
