...
Repair application and verification run in a separate offline sandbox. A candidate can become `VERIFIED_FOR_REVIEW`, never accepted. The human handoff runtime-validates the candidate; approval is impossible unless the embedded verifier summary is exactly `ACCEPTABLE_FOR_REVIEW` with passing tests and conformance, safe application, and zero new blocking findings. An approval record also requires an explicit human reviewer and immutable decision ID. High-risk work has no automatic acceptance path.

## Failure behavior

Provider cancellation, timeout, quota, rate limit, invalid JSON, unsupported citation, budget exhaustion, or offline mode returns a failed reasoner manifest. The deterministic ArchSync gate continues unchanged. Failure cannot downgrade BLOCK or promote REVIEW.
