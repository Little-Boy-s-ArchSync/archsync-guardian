...
| P4-114 | Five deterministic verifier outcomes are integrated. Only a complete `ACCEPTABLE_FOR_REVIEW` result can promote a candidate; no path approves, merges, or updates a baseline. | Human approval remains mandatory. |
| P4-124 | All 12 available attack/hard-negative safety cases run through the prompt, full outbound-context redaction, pre-call input budget, post-response usage/cost budget, citation, candidate-validation, or verification-bypass boundary with a deterministic fake provider. Failed budget manifests retain measured usage without raw response content. | P4-127 human security sign-off and a frozen real-provider configuration are required before this can become provider evidence. |

The inherited branch also retains the proposed P4-101/P4-102/P4-104–P4-107,
P4-121/P4-122, and P4-126 technical scaffolds. P4-115–P4-120, P4-123,
