...
| P4-112 | Allowlisted direct execution, timeout, bounded/sanitized logs, offline environment, and result classification are implemented behind the approved-isolator gate. A separate UNAPPROVED Docker candidate transfers bounded hashed project snapshots with no host mounts and denies network sockets including loopback; authored success/failure, process and cleanup probes are explicit. | Arbitrary repaired-workspace collection, dependency provisioning, an approved issuer and exact Security/Lead acceptance remain open. No arbitrary project test executes through the production verifier. Case-06 uses a non-spawning `TEST_ONLY` in-process invariant and remains non-reviewable. |
