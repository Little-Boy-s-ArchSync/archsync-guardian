...
The separate [offline provider evidence recorder](provider-evidence-recorder.md) can
serialize explicitly supplied request/response bytes and failed attempts into a
bounded, exclusive artifact. It is not connected to the provider runner. Its
synthetic tests demonstrate byte preservation, consistency checks and private
POSIX file publication; they do not approve real-provider storage, certify that
all sensitive content has been removed, or satisfy a retention/deletion policy.
- A disposable repair copy does not confine a hostile project test. Project commands are default-denied unless a module-issued, short-lived capability binds an approved filesystem-and-network isolator to the exact workspace. No production issuer is configured. Caller-forged, mismatched, expired, network-only, and `TEST_ONLY` capabilities cannot authorize a project-test spawn or a reviewable candidate.
